...
<div class="modal fade" id="archiveModal{{ $id->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body">
                <div class="d-flex justify-content-center">
                    <img src="{{ asset('img/unarchive.png') }}" alt="" style="width: 200px">
                </div>
                <div class="text-center vstack gap-3">
                    <div class="fw-bold text-secondary" style="font-size: 20px">Are you sure?</div>
                    <div class="text-secondary">This task will be restored from archive!</div>
                </div>
                <div class="d-flex gap-2 justify-content-center mt-3">
                    <form action="{{ route('archive.status.active', $id->id) }}" method="POST">
                        @csrf
                        @method('patch')
                        <input type="text" name="archive_status" value="active" hidden>
                        <a class="btn btn-danger" data-bs-dismiss="modal" style="font-size: 14px">No, cancel!</a>
                        <button class="btn btn-success" type="submit" style="font-size: 14px">Yes, proceed!</button>
                    </form>
                </div>
            </div>
        </div>
